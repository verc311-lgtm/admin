...
import { Search, ChevronRight, Download, X, FileText, FilePlus, Wallet, Anchor, Printer } from 'lucide-react';
...
const CompletedProjects: React.FC<CompletedProjectsProps> = ({ projects, invoices, payments }) => {
  ...
  const handleGeneratePDF = (project: Project) => {
    ...
    doc.text("Project Financial Report — Completed", pageWidth / 2, 28, { align: 'center' });
    ...
    doc.save(`Completed_${project.name.replace(/\s+/g, '_')}.pdf`);
  };
  ...
  const handleExportListPDF = () => {
    ...
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100);
    doc.text("Completed Projects Report", pageWidth / 2, 28, { align: 'center' });
    ...
    doc.setFont("helvetica", "bold");
    doc.setTextColor(100);
    doc.text("Total Contract Value", 20, 47);
    doc.text("Total Received", pageWidth / 2 - 20, 47);
    doc.text("Total Profit", pageWidth - 60, 47);

    doc.setFontSize(12);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(10, 25, 47);
    doc.text(`$${totalContract.toLocaleString()}`, 20, 54);
    doc.text(`$${totalReceived.toLocaleString()}`, pageWidth / 2 - 20, 54);
    doc.text(`$${totalProfit.toLocaleString()}`, pageWidth - 60, 54);

    const tableData = finishedProjects.map(project => {
      ...
      return [
        ...
        lastPayment ? `${lastPayment.date} ($${lastPayment.amount.toLocaleString()})` : '—'
      ];
    });

    autoTable(doc, {
      startY: 68,
      head: [['Project', 'Client', 'Contract', 'Paid', 'Profit', 'Final Payment']],
      body: tableData,
      theme: 'grid',
      headStyles: { fillColor: [16, 185, 129], textColor: 255, fontStyle: 'bold' },
      columnStyles: {
    ...
    doc.save(`Completed_Projects_${new Date().toISOString().split('T')[0]}.pdf`);
  };

  // Totals
  const totalContract = finishedProjects.reduce((s, p) => s + p.totalAmount, 0);
  const totalReceived = finishedProjects.reduce((s, p) => s + p.paidAmount, 0);
  const totalProfit = finishedProjects.reduce((s, p) => s + p.profit, 0);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="bg-white rounded-[2rem] md:rounded-[3rem] w-full max-w-4xl p-8 md:p-12 shadow-2xl relative max-h-[90vh] overflow-y-auto">
              <div className="flex justify-between items-start mb-8 md:mb-10 border-b pb-6">
                <div>
                  ...
                  <p className="text-emerald-600 font-bold uppercase tracking-widest text-[10px] mt-1">Completed Project — {p.client}</p>
                </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
                <div className="bg-slate-50 p-5 rounded-2xl"><p className="text-[9px] font-black uppercase text-slate-400 tracking-widest">Total Contract</p><p className="text-xl font-black">${p.totalAmount.toLocaleString()}</p></div>
                <div className="bg-emerald-50 p-5 rounded-2xl"><p className="text-[9px] font-black uppercase text-emerald-600 tracking-widest">Received</p><p className="text-xl font-black text-emerald-700">${p.paidAmount.toLocaleString()}</p></div>
                <div className="bg-orange-50 p-5 rounded-2xl"><p className="text-[9px] font-black uppercase text-orange-600 tracking-widest">Expenses</p><p className="text-xl font-black text-orange-600">${(p.totalExpenses || 0).toLocaleString()}</p></div>
                <div className="bg-blue-50 p-5 rounded-2xl"><p className="text-[9px] font-black uppercase text-blue-600 tracking-widest">Profit Margin</p><p className="text-xl font-black">{profitPercent.toFixed(1)}%</p></div>
              </div>

              {/* Payment History */}
              {projectPayments.length > 0 && (
                <div className="mb-8">
                  <h4 className="text-lg font-black uppercase italic flex items-center gap-2 mb-4"><FileText className="w-5 h-5 text-cyan-600" /> Payment History</h4>
                  <div className="bg-slate-50 rounded-2xl overflow-hidden border">
                    <table className="w-full text-left">
                      <thead className="bg-slate-100/50"><tr><th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Date</th><th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400">Method</th><th className="px-6 py-4 text-[10px] font-black uppercase tracking-widest text-slate-400 text-right">Amount</th></tr></thead>
                      <tbody className="divide-y">
                        {projectPayments.map(pay => (
                          <tr key={pay.id} className="hover:bg-white transition-colors">
                            ...
                            <td className="px-6 py-4"><span className={`px-2.5 py-1 text-[10px] font-black rounded-full uppercase ${pay.method === 'Discount' ? 'bg-amber-50 text-amber-600' : 'bg-blue-50 text-blue-600'}`}>{pay.method}</span></td>
                            <td className={`px-6 py-4 text-right font-black ${pay.method === 'Discount' ? 'text-amber-600' : 'text-emerald-600'}`}>{pay.method === 'Discount' ? '-' : ''}${pay.amount.toLocaleString()}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
      ...
      <div className="bg-white p-6 md:p-8 rounded-[2.5rem] shadow-sm border flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6 sticky top-0 z-10">
        <div className="flex items-center gap-4 flex-shrink-0">
          ...
          <div>
            <h2 className="text-xl md:text-2xl font-black text-[#0a192f] uppercase tracking-tighter italic leading-none">Completed Projects</h2>
            <p className="text-emerald-600 text-[10px] font-bold uppercase tracking-widest mt-0.5">{finishedProjects.length} finished project{finishedProjects.length !== 1 ? 's' : ''}</p>
          </div>
        </div>
        <div className="relative flex-1 w-full flex flex-col md:flex-row gap-4 items-center">
          <div className="relative flex-1 w-full">
            ...
            <input type="text" placeholder="Search by project or client..." className="w-full pl-14 pr-6 py-4 rounded-2xl bg-slate-50 outline-none font-bold text-slate-700 focus:bg-white focus:ring-4 focus:ring-emerald-50 transition-all text-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
          <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-[#0a192f] to-slate-800 p-6 rounded-[2rem] text-white">
          <p className="text-[10px] font-bold uppercase tracking-widest text-cyan-400 mb-2">Total Contract Value</p>
        ...
        <div className="bg-gradient-to-br from-blue-500 to-blue-600 p-6 rounded-[2rem] text-white">
          <p className="text-[10px] font-bold uppercase tracking-widest text-blue-100 mb-2">Total Profit</p>
          <p className="text-3xl font-black">${totalProfit.toLocaleString()}</p>
        </div>
      </div>
      ...
      {finishedProjects.length === 0 ? (
        <div className="bg-white rounded-[2.5rem] shadow-sm border p-16 text-center">
          <p className="text-slate-300 text-lg font-bold">No completed projects found</p>
          <p className="text-slate-400 text-sm mt-2">Projects marked as "Finished" will appear here.</p>
        </div>
      ) : (
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden">
          ...
          <div className="md:hidden p-4 space-y-3">
            {finishedProjects.map(project => {
              ...
              return (
                <div key={project.id} onClick={() => setDetailsModal({ show: true, project })} className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden active:scale-[0.98] transition-transform cursor-pointer">
                  ...
                  <div className="p-4">
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-3">
                      ...
                      <div className="bg-blue-50 p-2.5 rounded-xl">
                        <p className="text-[8px] font-black text-blue-600 uppercase">Profit</p>
                        <p className={`text-sm font-black ${profitPercent >= 30 ? 'text-emerald-600' : profitPercent >= 15 ? 'text-amber-600' : 'text-red-500'}`}>{profitPercent.toFixed(1)}%</p>
                      </div>
                    </div>
                    {lastPayment && (
                      <div className="border-t border-slate-100 pt-3 flex justify-between items-center">
                        <div>
                          <p className="text-[8px] font-black text-slate-400 uppercase">Final Payment</p>
                ...
                {finishedProjects.map(project => {
                  ...
                  return (
                    <tr key={project.id} className="hover:bg-emerald-50/20 transition-colors group">
                      ...
                      <td className="px-6 py-5">
                        {lastPayment ? (
                          <span className="px-2.5 py-1 bg-blue-50 text-blue-600 text-[10px] font-black rounded-full uppercase">{lastPayment.method}</span>
                        ) : (
                          <span className="text-xs text-slate-300">—</span>
                        )}
                      </td>
