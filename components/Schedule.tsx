...
// Vibrant Monday.com-style color palette for projects
...
const Schedule: React.FC<ScheduleProps> = ({
    ...
    // Project color map (stable by project index)
    ...
    const handleSaveDates = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingProject && editForm.start) { onUpdateDates(editingProject.id, editForm.start, editForm.end || undefined); setEditingProject(null); }
    };
    ...
    const handleSaveAssignment = (e: React.FormEvent) => {
        ...
        if (selectedCell && assignmentForm.projectId) {
            selectedDays.forEach(day => onAddAssignment({
                ...
                date: day.toISOString().split('T')[0], activity: assignmentForm.activity,
                workers: assignmentForm.workers, status: 'Pending'
            }));
    ...
    const shareScheduleWhatsApp = () => {
        ...
        crews.forEach(crew => {
            const ca = assignments.filter(a => { const d = new Date(a.date); return a.crewId === crew.id && d >= weekStart && d <= weekEnd; });
            if (ca.length > 0) {
    ...
    return (
        <div className="space-y-5 pb-10">
            ...
            {/* ── HERO HEADER ── */}
            <div className="bg-[#0a192f] rounded-[2rem] p-8 text-white relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
                    <div>
                        <div className="flex items-center gap-3 mb-1">
                            <div className="bg-cyan-500/20 p-2 rounded-xl"><CalendarDays className="w-6 h-6 text-cyan-400" /></div>
                            <h2 className="text-2xl font-black tracking-tight">Project Schedule</h2>
                        </div>
                        <p className="text-slate-400 text-sm ml-1">Plan, track, and manage your operations</p>
                        ...
                        <div className="flex gap-6 mt-5">
                            ...
                            <div className="flex flex-col">
                                <span className="text-3xl font-black text-emerald-400">{doneCount}</span>
                                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Done</span>
                            </div>
                            <div className="w-px bg-slate-700" />
                            <div className="flex flex-col">
                                <span className="text-3xl font-black text-rose-400">{delayedCount}</span>
                                <span className="text-[10px] text-slate-400 uppercase tracking-widest font-bold">Delayed</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        ...
                        <div className="flex bg-white/10 p-1 rounded-xl backdrop-blur-sm border border-white/10">
                            <button onClick={() => setViewMode('Timeline')} className={`px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all flex items-center gap-2 ${viewMode === 'Timeline' ? 'bg-white text-[#0a192f] shadow' : 'text-slate-300 hover:text-white'}`}>
                                <BarChart2 className="w-3.5 h-3.5" /> Gantt
                            </button>
                            <button onClick={() => setViewMode('Weekly')} className={`px-4 py-2 text-xs font-bold uppercase tracking-wider rounded-lg transition-all flex items-center gap-2 ${viewMode === 'Weekly' ? 'bg-white text-[#0a192f] shadow' : 'text-slate-300 hover:text-white'}`}>
                        ...
                        {viewMode === 'Weekly' && (
                            <>
                                <button onClick={() => setShowCrewModal(true)} title="Add Team" className="p-2.5 bg-white/10 hover:bg-white/20 backdrop-blur-sm border border-white/10 text-white rounded-xl transition-all">
                                    <Plus className="w-4 h-4" />
                                </button>
                                <button onClick={shareScheduleWhatsApp} title="Share on WhatsApp" className="p-2.5 bg-emerald-500/20 hover:bg-emerald-500/30 backdrop-blur-sm border border-emerald-500/20 text-emerald-300 rounded-xl transition-all">
                        ...
                        <div className="flex items-center gap-1 bg-white/10 backdrop-blur-sm border border-white/10 rounded-xl p-1">
                            ...
                            <span className="text-xs font-bold text-white min-w-[120px] text-center py-1 uppercase tracking-wide">
                                {viewMode === 'Timeline' ? monthName : `${days[0].toLocaleDateString('default', { month: 'short', day: 'numeric' })} – ${days[6].toLocaleDateString('default', { month: 'short', day: 'numeric' })}`}
                            </span>
            ...
            {viewMode === 'Timeline' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    {/* Column header */}
                    <div className="grid border-b border-slate-100" style={{ gridTemplateColumns: '260px 1fr' }}>
                        ...
                        <div className="overflow-x-auto">
                            <div className="flex min-w-max">
                                {days.map(day => {
                                    ...
                                    return (
                                        <div key={day.toISOString()} className={`w-10 flex-shrink-0 border-r border-slate-100 last:border-r-0 ${isWeekend ? 'bg-slate-50' : ''}`}>
                    ...
                    <div className="divide-y divide-slate-100">
                        {projects.length === 0 && (
                            <div className="p-16 text-center text-slate-300">
                                ...
                                <p className="font-bold text-sm">No projects scheduled yet.</p>
                            </div>
                        )}
                        {projects.map((project, pidx) => {
                            ...
                            return (
                                <div key={project.id} className="grid hover:bg-slate-50/60 transition-colors group" style={{ gridTemplateColumns: '260px 1fr' }}>
                                    {/* Left: Project info */}
                                    <div
                                        className="p-4 border-r border-slate-100 flex flex-col justify-center cursor-pointer"
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-2 ml-[22px]">
                                            <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold border flex items-center gap-1 ${status.cls}`}>
                                    ...
                                    <div className="overflow-x-auto relative">
                                        <div className="flex min-w-max h-full relative">
                                            {days.map(day => {
                                                ...
                                                const isToday = day.toDateString() === today.toDateString();
                                                return (
                                                    <div key={day.toISOString()} className={`w-10 flex-shrink-0 border-r border-slate-50 last:border-r-0 min-h-[80px] ${isWeekend ? 'bg-slate-50/50' : ''} ${isToday ? 'bg-cyan-50/30' : ''}`} />
                                                );
                                            })}

                                            <div className="absolute inset-0 flex items-center min-w-max pointer-events-none px-1">
                                                {(() => {
                                                    ...
                                                    return (
                                                        <div
                                                            className="absolute h-9 rounded-xl flex items-center px-3 overflow-hidden shadow-md"
                                                            style={{
                                                                left: `${si * 40 + 4}px`,
                                                                width: `${Math.max(dur * 40 - 8, 40)}px`,
                                                                backgroundColor: color.hex,
                                                                backgroundImage: `linear-gradient(135deg, ${color.hex}ee, ${color.hex}bb)`,
                                                            }}
                                                        >
                                                            {/* Progress fill overlay */}
                                                            <div className="absolute inset-0 left-0 rounded-xl opacity-30 bg-white" style={{ width: `${progress}%` }} />
                                                            <div className="relative flex items-center gap-2 w-full">
                                                                {delay > 0 && <AlertTriangle className="w-3.5 h-3.5 text-white flex-shrink-0" />}
                    ...
                    {/* Legend */}
                    <div className="px-6 py-3 bg-slate-50 border-t border-slate-100 flex flex-wrap gap-4 items-center">
                        {Object.entries(STATUS_CONFIG).map(([, v]) => (
                            <div key={v.label} className={`flex items-center gap-1.5 text-[10px] font-bold px-2.5 py-1 rounded-full border ${v.cls}`}>
                                {v.icon} {v.label}
                            </div>
                        ))}
                        <span className="text-[10px] text-slate-400 ml-auto italic">Click any project row to edit dates</span>
                    </div>
                </div>
            )}
            ...
            {viewMode === 'Weekly' && (
                <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                    ...
                    <div className="grid border-b border-slate-100" style={{ gridTemplateColumns: '160px repeat(7, 1fr)' }}>
                        <div className="px-4 py-3 bg-slate-50 border-r border-slate-100 flex items-center">
                        ...
                        {days.map(day => {
                            ...
                            const isWeekend = day.getDay() === 0 || day.getDay() === 6;
                            return (
                                <div key={day.toISOString()} className={`p-3 text-center border-r border-slate-100 last:border-r-0 relative ${isToday ? 'bg-cyan-50' : isWeekend ? 'bg-slate-50/70' : 'bg-slate-50'}`}>
                                    {isToday && <div className="absolute top-0 left-0 right-0 h-0.5 bg-cyan-500" />}
                                    <div className={`text-[10px] font-black uppercase tracking-widest ${isToday ? 'text-cyan-600' : 'text-slate-400'}`}>
                                        {day.toLocaleString('default', { weekday: 'short' })}
                                    </div>
                                    <div className={`text-lg font-black mt-0.5 ${isToday ? 'text-cyan-600' : isWeekend ? 'text-slate-400' : 'text-slate-700'}`}>{day.getDate()}</div>
                                </div>
                            );
                        })}
                    </div>
                    ...
                    {crews.length === 0 ? (
                        <div className="p-16 text-center">
                            ...
                            <p className="text-slate-400 font-bold">No teams added yet.</p>
                            <button onClick={() => setShowCrewModal(true)} className="mt-4 px-6 py-2.5 bg-cyan-600 hover:bg-cyan-500 text-white text-xs font-bold uppercase tracking-wide rounded-xl transition-all flex items-center gap-2 mx-auto">
                                <Plus className="w-4 h-4" /> Add First Team
                            </button>
                        </div>
                    ) : (
                        crews.map((crew, cidx) => (
                            <div key={crew.id} className="grid border-b border-slate-100 last:border-b-0" style={{ gridTemplateColumns: '160px repeat(7, 1fr)', minHeight: '110px' }}>
                                {/* Crew label */}
                                <div className="px-4 py-4 border-r border-slate-100 flex flex-col justify-center bg-gradient-to-r from-slate-50 to-white">
                                    <div className="flex items-center gap-2 mb-1">
                                        <div className="w-2 h-8 rounded-full" style={{ backgroundColor: PROJECT_COLORS[cidx % PROJECT_COLORS.length].hex }} />
                                        <div>
                                            <div className="font-black text-slate-700 text-sm">{crew.name}</div>
                                            <div className="text-[10px] text-slate-400 font-medium">
                                                {assignments.filter(a => {
                                                    const d = new Date(a.date);
                                                    return a.crewId === crew.id && d >= days[0] && d <= days[6];
                                                }).length} tasks this week
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                {days.map(day => {
                                    ...
                                    return (
                                        <div
                                            key={day.toISOString()}
                                            className={`border-r border-slate-100 last:border-r-0 p-2 cursor-pointer transition-all relative group ${isWeekend ? 'bg-slate-50/50' : ''} ${isToday ? 'bg-cyan-50/30' : ''} ${!assignment ? 'hover:bg-slate-50' : ''}`}
                                            onClick={() => {
                                                ...
                                                setAssignmentForm(assign ? { id: assign.id, projectId: assign.projectId, activity: assign.activity, workers: assign.workers || '' } : { id: '', projectId: '', activity: '', workers: '' });
                                            }}
                                        >
                                            {assignment && color && proj ? (
                                                <div className={`h-full w-full rounded-xl p-2.5 ${color.bgLight} border ${color.border} flex flex-col gap-1 shadow-sm hover:shadow-md transition-shadow`}>
                                                    <div className={`text-[10px] font-black uppercase tracking-wider ${color.text} truncate flex items-center gap-1`}>
                                                        <div className={`w-1.5 h-1.5 rounded-full ${color.bg}`} />
                                                        {proj.name}
                                                    </div>
                                                    <div className="text-[11px] font-semibold text-slate-700 leading-tight line-clamp-2">{assignment.activity}</div>
                                                    {assignment.workers && (
                    ...
                    <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSaveDates} className="p-6 space-y-4">
                            ...
                            <div className="flex gap-3 pt-2">
                                ...
                                <button type="submit" className="flex-1 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-lg shadow-cyan-600/20 transition-all">Save Schedule</button>
                            </div>
                        </form>
                    </div>
                </div>
                ...
                <div className="fixed inset-0 bg-[#0a192f]/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => { setSelectedCell(null); resetForm(); }}>
                    <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        <div className="p-6 bg-gradient-to-br from-[#0a192f] to-[#112240] text-white">
                            <div className="flex justify-between items-start">
                                <div>
                                    <p className="text-cyan-400 text-xs font-bold uppercase tracking-widest mb-1">{assignmentForm.id ? 'Edit Assignment' : 'New Assignment'}</p>
                                    <h3 className="font-black text-xl">{crews.find(c => c.id === selectedCell.crewId)?.name}</h3>
                                    <p className="text-slate-400 text-sm mt-0.5">{selectedCell.date.toLocaleDateString('default', { weekday: 'long', month: 'long', day: 'numeric' })}</p>
                                </div>
                        ...
                        <form onSubmit={handleSaveAssignment} className="p-6 space-y-4">
                                    ...
                                    {days.map(day => {
                                        ...
                                        return (
                                            <button key={day.toISOString()} type="button"
                                                ...
                                                className={`px-3 py-2 rounded-xl text-xs font-bold border-2 transition-all ${isSel ? 'bg-cyan-600 border-cyan-600 text-white shadow-md shadow-cyan-600/20' : 'bg-slate-50 border-slate-200 text-slate-500 hover:border-cyan-300 hover:bg-cyan-50'}`}>
                                                {day.toLocaleDateString('default', { weekday: 'short' })} {day.getDate()}
                                            </button>
                                        );
                                    })}
                                </div>
                                <p className="text-[10px] text-slate-400 mt-1.5">* Select multiple days for bulk assignment</p>
                            </div>
                            ...
                            <div>
                                ...
                                <input type="text" placeholder="e.g. John, Mike, Steve"
                                    className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-200 rounded-xl focus:border-cyan-500 focus:outline-none font-bold text-slate-700"
                            ...
                            <div className="flex gap-3 pt-2">
                                {assignmentForm.id && (
                                    <button type="button" onClick={() => { if (window.confirm('Delete?')) { onDeleteAssignment(assignmentForm.id); setSelectedCell(null); resetForm(); } }}
                                        className="p-3 bg-rose-50 hover:bg-rose-100 text-rose-600 font-bold rounded-xl transition-all border-2 border-rose-100">
                                        <Trash2 className="w-4 h-4" />
                                    </button>
                                )}
                                <button type="button" onClick={() => { setSelectedCell(null); resetForm(); }} className="flex-1 px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl transition-all">Cancel</button>
                                <button type="submit" className="flex-1 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-lg shadow-cyan-600/20 transition-all">
                ...
                <div className="fixed inset-0 bg-[#0a192f]/80 backdrop-blur-sm z-[100] flex items-center justify-center p-4" onClick={() => setShowCrewModal(false)}>
                    <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full overflow-hidden animate-in fade-in zoom-in-95 duration-200" onClick={e => e.stopPropagation()}>
                        ...
                        <form onSubmit={handleSaveCrew} className="p-6 space-y-4">
                            ...
                            <div className="flex gap-3 pt-2">
                                <button type="button" onClick={() => setShowCrewModal(false)} className="flex-1 px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-600 font-bold rounded-xl transition-all">Cancel</button>
                                <button type="submit" className="flex-1 px-6 py-3 bg-cyan-600 hover:bg-cyan-500 text-white font-bold rounded-xl shadow-lg shadow-cyan-600/20 transition-all">Create Team</button>
                            </div>
                        </form>
                    </div>
                </div>
